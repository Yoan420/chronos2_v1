<#
.SYNOPSIS
NYX + four hourly solar forecasts. Separate experiment, operational modes unchanged.
#>
[CmdletBinding()]
param(
    [ValidateSet('Run','Prepare','Sync','Audit','Report','Status')][string]$Action = 'Run',
    [string]$Config = 'config\solar_cwe.yaml',
    [string]$DeliveryDay,
...
$SolarScript = Join-Path $SolarProject 'run_solar_cwe_forecast.py'
...
if ($LASTEXITCODE -ne 0) { throw "SolarCWE a retourne le code $LASTEXITCODE. La production reste inchangee." }
