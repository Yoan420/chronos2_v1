"""Isolated, dated Test2 execution: SolarWind + corrector-only interaction ±40.

No source synchronizer, production writer, or mutation of an imported module's
globals is used. A nested project root satisfies the unchanged SolarWind path
...
ENGINE = 'nyx_test2_live_v1'
OUTPUT = ROOT / 'runs/experiments' / ENGINE
...
IMPLEMENTATION = ('chronos2_hourly/nyx_live_baseline.py',
...
def _inputs(zone, day, nuclear_workdir, sources, workroot, threads, workers, device, builder):
    ...
    if zone not in TIMEZONES or threads not in (1, 2) or workers not in (1, 2, 3, 4) or device not in ('cpu', 'auto', 'cuda'):
        raise ValueError('CWE, one-to-two threads and one-to-four workers required')
    ...
    mini = root / day / zone.lower() / identity['run_id']
    work = safe_path(mini / 'runs/experiments/solar_wind_v1/result')
...
def _prepare(identity, work, mini, donor, bundle, prior, config):
    ...
    config['hourly']['residual_correction']['thread_count'] = identity['threads']
    ...
    experiment.update(input_protocol=solar_wind_input_protocol(), mode='incremental',
        incremental_cache_dir=str(mini / 'runs/experiments/solar_wind_v1/_daily_cache' / zone.lower()),
...
def run_zone(zone, delivery_day, nuclear_workdir, sources, *, workroot=OUTPUT,
             threads=1, workers=1, device='cpu', action='run', interaction_builder=None):
    ...
    with exclusive_process_lock(safe_path(Path(workroot) / '_batch_locks' / f'{day}_{zone}.lock')):
        ...
        try:
            ...
            result = replace(result, audit=dict(result.audit, nyx_test2_identity=identity,
