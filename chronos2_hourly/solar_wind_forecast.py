"""Four hourly solar forecasts and two hourly wind forecasts added to the unchanged NuclearFR model chain.
...
six additional audited GW inputs. Source materialization belongs to the caller.
"""
...
ENGINE = "solar_wind_v1"
GENERATION_SERIES = {f"{zone}_solar_generation_fcst": f"power.{zone}.generation.solar.hourly.gw.fcst"
                for zone in ("fr", "de", "be", "nl")}
GENERATION_SERIES.update({f"{zone}_wind_generation_fcst": f"power.{zone}.generation.wind.hourly.gw.fcst"
                          for zone in ("de", "nl")})
GENERATION_ALIASES = tuple(GENERATION_SERIES)
GENERATION_KNOWN_COLUMNS = tuple(f"known_{alias}_oracle" for alias in GENERATION_ALIASES)
INPUT_ALIASES = (*BASE_RESIDUAL_LOAD_COVARIATES, base.NUCLEAR_ALIAS, *GENERATION_ALIASES)


class SolarWindForecastError(base.NuclearForecastError):
    """An incomplete input, unintended schema or non-isolated path was refused."""


def solar_wind_code_sha256() -> str:
    return hashlib.sha256(Path(__file__).read_bytes()).hexdigest()


def solar_wind_input_protocol() -> str:
    contract = {"engine": ENGINE, "adapter_sha256": solar_wind_code_sha256(), "sources": GENERATION_SERIES,
                "units": "GW", "daily_broadcast": False, "cutoff": "civil_D_minus_1_08:00"}
    return "civil_pit_solar_wind_v1_" + hashlib.sha256(json.dumps(contract, sort_keys=True).encode()).hexdigest()


def solar_wind_kalman_covariate_config(original: KalmanCovariateConfig | None = None) -> KalmanCovariateConfig:
    original = original or base.nuclear_kalman_covariate_config()
    result = replace(original,
        input_columns=tuple(dict.fromkeys((*original.input_columns, *GENERATION_ALIASES))),
        groups={name: tuple(dict.fromkeys((*columns, *GENERATION_ALIASES))) for name, columns in original.groups.items()},
        history_missing_policy="complete_trailing", minimum_history_coverage=1.0, require_future_complete=True)
    # Keep residual-load mean/spread unchanged: generation is neither a new aggregate
...
def _isolated_paths(config: Mapping[str, Any], workdir: str | Path) -> None:
    ...
    if (allowed.resolve() != allowed or work.resolve() != work or not work.is_relative_to(allowed)
        ...
        raise SolarWindForecastError("SolarWind workdir must be an unsealed, unredirected child of runs/experiments/solar_wind_v1.")
    ...
    if experiment.get("mode", "full") == "incremental":
        ...
        if cache.resolve() != cache or not cache.is_relative_to(allowed) or cache == allowed:
            raise SolarWindForecastError("SolarWind daily caches must belong to the isolated SolarWind namespace.")
        namespace = Path(str(experiment.get("incremental_namespace", cache))).absolute()
        if namespace.resolve() != namespace or not namespace.is_relative_to(cache):
            raise SolarWindForecastError("SolarWind cache namespace must stay inside its isolated cache root.")
...
def _prepare_inputs(config: Mapping[str, Any], data: Any, zone: str, day: date):
    ...
    if zone not in {"FR", "DE", "BE", "NL"} or data.zone != zone or data.timezone != base.ZONE_TIMEZONES[zone]:
        raise SolarWindForecastError("Prepared SolarWind zone/timezone must match FR/DE/BE/NL.")
    ...
    if active != set(INPUT_ALIASES):
        raise SolarWindForecastError("SolarWind accepts exactly baseline RL5 + FR nuclear + four solar and two wind inputs, no additional raw features.")
    for alias, series in GENERATION_SERIES.items():
        ...
        if (spec.get("enabled") is not True or spec.get("source") != "pit_parquet" or spec.get("series") != series
            ...
            raise SolarWindForecastError(f"{alias}: exact hourly GW generation PIT, unfilled base context and known future required.")
    context = _frame(data.model_context_covariates, "SolarWind model context")
    ...
    if (set(context) - allowed_columns or set(data.known_future_columns) - allowed_future
        ...
        raise SolarWindForecastError("Unexpected model input: no realised generation, ramp, classifier, fuel or competing forecast is allowed.")
    if config.get("hourly", {}).get("supply_stack", {}).get("enabled", False):
        raise SolarWindForecastError("SolarWind does not enable additional supply-stack features.")
    ...
    if (pd.isna(start) or start.tzinfo is not None or start != start.normalize()
        ...
        raise SolarWindForecastError("SolarWind raw history requires the pinned 730-to-1095-day replay anchor.")
    ...
    if len(expected.difference(context.index)):
        raise SolarWindForecastError("SolarWind context misses replay or future physical hours.")
    target, _, future, features = _feature_inputs(data, config)
    if not required_known.issubset(features):
        raise SolarWindForecastError("Baseline or solar known-future input dropped from residual feature selection.")
    history = _frame(data.covariates, "SolarWind historical covariates")
    ...
    for alias in GENERATION_ALIASES:
        ...
        if (alias not in context or alias not in history or known not in context or known not in data.known_future_columns
            ...
            raise SolarWindForecastError(f"{alias}: input dropped from Chronos context/future or residual features.")
        ...
        if not np.isfinite(mandatory.to_numpy(float)).all() or np.isinf(values.to_numpy(float)).any():
            raise SolarWindForecastError(f"{alias}: every replay/future physical hour must be finite.")
        ...
        if (not np.array_equal(finite[:, 0], finite[:, 1]) or not np.allclose(values.loc[paired, alias], values.loc[paired, known], rtol=0, atol=1e-9)
            ...
            raise SolarWindForecastError(f"{alias}: negative generation or divergent base/future vintages.")
        ...
        if (not np.isfinite(historical_values).all() or not np.isfinite(selected).all()
            ...
            raise SolarWindForecastError(f"{alias}: historical or residual values diverge from the PIT input.")
...
class _SolarWindKalmanBuilder:
    ...
    def __call__(self, **kwargs):
        incoming = _frame(kwargs["covariates"], "SolarWind Kalman covariates")
        for alias in GENERATION_ALIASES:
        ...
        if set(incoming) != set(INPUT_ALIASES) or not np.isfinite(incoming.to_numpy(float)).all():
            raise SolarWindForecastError("SolarWind Kalman must retain complete RL5/nuclear/four-solar/two-wind inputs only.")
        self.used_config = solar_wind_kalman_covariate_config(kwargs["covariate_config"])
...
def run_solar_wind_forecast(*, config: Mapping[str, Any], data: Any, zone: str, delivery_day: str | date,
    ...
    """Recompute the unchanged NuclearFR chain with six added solar/wind channels."""
    ...
    if resolved.get("nuclear_experiment", {}).get("input_protocol") != solar_wind_input_protocol():
        raise SolarWindForecastError("SolarWind input_protocol must pin the current adapter and six-source schema.")
    parsed = pd.Timestamp(delivery_day)
    if pd.isna(parsed) or parsed.tzinfo is not None or parsed != parsed.normalize():
        raise SolarWindForecastError("SolarWind delivery_day must be an explicit civil date.")
    ...
    if factory is None or upstream != "chronos2":
        raise SolarWindForecastError("SolarWind requires the unchanged enabled Chronos-based residual recipe.")
    ...
    required = (*GENERATION_KNOWN_COLUMNS, base.NUCLEAR_KNOWN_COLUMN)
    if not builder.exclude_historical_prices or any(builder._is_excluded(column) for column in required):
        raise SolarWindForecastError("Residual recipe must retain generation/nuclear inputs and its historical-price exclusion.")
    solar_wind_kalman_covariate_config()  # Dimension/group validation before any costly model call.
    ...
    hook = _SolarWindKalmanBuilder(context, kalman_builder or build_operational_kalman_view, signature)
    ...
    if hook.used_config is None or hook.used_covariates is None:
        raise SolarWindForecastError("SolarWind Kalman input adapter was not executed.")
    daily = result.residual_daily_audit
    if not {"generation_source", "residual_feature_columns"}.issubset(daily):
        raise SolarWindForecastError("Fitted residual-feature audit is required.")
    ...
    if fitted.empty or any(not set(required).issubset(columns) for columns in fitted):
        raise SolarWindForecastError("A fitted residual corrector dropped a required generation/nuclear input.")
    if str(result.audit.get("raw_history_start_day")) != expected[0].tz_convert(data.timezone).date().isoformat():
        raise SolarWindForecastError("SolarWind engine changed the pinned replay anchor.")
    audit = deepcopy(dict(result.audit))
    audit.update(candidate_engine=ENGINE, candidate_variant="solar_wind_kalman", solar_wind_input_protocol=solar_wind_input_protocol(),
        solar_wind_adapter_sha256=solar_wind_code_sha256(), solar_wind_input_coverage=coverage, solar_wind_kalman_delegate_signature=signature,
        solar_wind_sources={alias: {"series": series, "unit": "GW", "semantic": "forecast_generation", "daily_broadcast": False}
                       for alias, series in GENERATION_SERIES.items()},
        solar_wind_chronos_context_columns=list(GENERATION_ALIASES), solar_wind_chronos_known_future_columns=list(GENERATION_KNOWN_COLUMNS),
        solar_wind_residual_features=list(GENERATION_KNOWN_COLUMNS), solar_wind_kalman_market_features=list(GENERATION_ALIASES),
        kalman_covariate_config=hook.used_config.to_dict(), baseline_inputs_preserved=True,
        additional_raw_input_count=6, new_ramps_or_aggregates=False, spike_classifier_added=False,
        inherited_residual_feature_builder_unchanged=True, production_pit_evidence=False,
        source_audit_responsibility="caller_pinned_hourly_solar_wind_PIT_sources_and_civil_D_minus_1_08_cutoff_audits",
...
__all__ = ["ENGINE", "GENERATION_SERIES", "GENERATION_ALIASES", "GENERATION_KNOWN_COLUMNS", "INPUT_ALIASES", "SolarWindForecastError",
           "solar_wind_code_sha256", "solar_wind_input_protocol", "solar_wind_kalman_covariate_config", "run_solar_wind_forecast"]
