...
def _local_naive_index(
    index: pd.DatetimeIndex,
) -> pd.DatetimeIndex:
    """Retourne l'heure murale locale sans réinterpréter le fuseau."""
    values = pd.DatetimeIndex(index)
    return values.tz_localize(None) if values.tz is not None else values


def _price_lag_series(
    target: pd.Series,
    index: pd.DatetimeIndex,
    days: int,
) -> pd.Series:
    """Prix du même créneau sur un jour local antérieur, robuste au DST.

    On ne soustrait jamais ``DateOffset`` à un timestamp Europe/Paris :
    l'heure 02:00 du passage à l'heure d'hiver est ambiguë. La jointure est
    faite sur la date locale et l'heure murale. Si le jour source contient
    deux occurrences du même créneau, leur moyenne est utilisée. Si le
    créneau n'existe pas au passage à l'heure d'été, la feature reste NaN.
    """
    if days <= 0:
        raise ValueError("days doit être strictement positif.")

    delivery_index = pd.DatetimeIndex(index)
    source_index = pd.DatetimeIndex(target.index)
    source_local = _local_naive_index(source_index)
    delivery_local = _local_naive_index(delivery_index)

    source_frame = pd.DataFrame(
        {
            "local_date": source_local.normalize(),
    ...
    # Le 30/10/2022 à 02:00 existe deux fois. Une moyenne évite de choisir
    # arbitrairement l'occurrence été ou hiver et conserve une ligne par
    # créneau local.
    lookup = source_frame.groupby(
        ["local_date", "hour", "minute", "second"],
        sort=False,
    )["value"].mean()

    previous_date = (
        delivery_local.normalize() - pd.Timedelta(days=days)
    )
    query = pd.MultiIndex.from_arrays(
        [
            previous_date,
            delivery_local.hour,
            delivery_local.minute,
            delivery_local.second,
        ],
        names=["local_date", "hour", "minute", "second"],
    )

    values = lookup.reindex(query).to_numpy(dtype=np.float64)
    return pd.Series(
        values,
        index=delivery_index,
        name=f"price_lag_{days}d",
        dtype=np.float64,
    )
...
def _price_lag(
    ...
    return _price_lag_series(target, index, days).to_numpy(
        dtype=np.float64
    )
...
def _add_price_history_features(
    ...
    previous_day = _price_lag_series(
        target, delivery_index, 1
    ).dropna()
...
def feature_columns(frame: pd.DataFrame) -> list[str]:
    excluded_columns = set(METADATA_COLUMNS) | set(LABEL_COLUMNS)
