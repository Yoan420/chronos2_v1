...
_SOURCE_KEYS = {"timezone", "autonomous_run", "checksum_manifest_sha256"}
_TOPOLOGY_KEYS = {
    "radii",
    "adjacency",
    "residual_load",
...
def _parse_source(
    ...
    *,
    ...
    return SealedAutonomousSource(
        zone, tz, run_dir, checksum_manifest, configured_sha,
        backtest, backtest_sha, aligned, aligned_sha,
        run_manifest, run_manifest_sha, feature_manifest, feature_manifest_sha,
        recipe, recipe_sha, metrics, metrics_sha,
    )
...
def _parse_blend_dependency(
    ...
    *,
    ...
    if not manifest.is_file() or not forecast.is_file():
        raise TopologyContractError(f"{zone}: missing blend dependency or forecast")
    if verify_hashes:
        if _sha256_file(manifest) != manifest_sha:
            raise TopologyContractError(f"{zone}: blend dependency hash mismatch")
        if _sha256_file(forecast) != forecast_sha:
            raise TopologyContractError(f"{zone}: blend forecast hash mismatch")
    payload = _load_json(manifest, name=f"{zone} blend dependency")
    if payload.get("dependency_gate_passed") is not True:
        raise TopologyContractError(f"{zone}: blend dependency gate failed")
    if payload.get("storm_token_found") is not False:
        raise TopologyContractError(f"{zone}: blend dependency is not Storm-free")
    if str(payload.get("terminal_series")) != primary:
        raise TopologyContractError(f"{zone}: blend terminal series mismatch")
    return BlendDependency(zone, manifest, manifest_sha, forecast, forecast_sha, primary)
...
def load_topology_contract(
    ...
    *,
    ...
    if autonomous["native_model"] != "topology_autonomous" or autonomous["baseline_model"] != "identity":
    ...
    expected_blend_scalars = {
        ...
        "baseline_model": "topology_autonomous",
    ...
    for zone in BLEND_ZONES:
        ...
        dependencies[zone] = _parse_blend_dependency(
            zone, raw_dependencies[key], project_root=root, verify_hashes=verify_hashes
        )
...
def audit_blend_source(
    ...
    """Audit a sealed MKOnline input after rejecting non-FR/NL zones first."""

    code = _zone(zone)
    dependency = contract.blend_dependency_for(code)  # eligibility before I/O
    try:
        import numpy as np
        import pandas as pd
    except ImportError as exc:  # pragma: no cover
