"""Separate CoherentP50 experiments, with no operational writes or promotion."""
...
def main(argv=None):
    ...
    parser.add_argument("--config", type=Path, default=ROOT/"config/nyx_coherent_p50.yaml")
    ...
    try:
        ...
        if args.action == "worker" and (not args.variant or args.run_directory is None):
            raise ValueError("A worker requires its sealed --run-directory and --variant.")
        ...
        logging.exception("CoherentP50 experiment failed. Existing operational forecasts remain unchanged.")
        return 2
