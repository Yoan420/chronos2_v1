"""Dedicated physical P50 laboratory; never invokes production forecasts."""
...
def main(argv=None):
    ...
    parser.add_argument("--config", type=Path, default=ROOT/"config/nyx_physical_p50.yaml")
    parser.add_argument("--run-directory")
    parser.add_argument("--start-day")
    parser.add_argument("--end-day")
    parser.add_argument("--ca-bundle")
    args = parser.parse_args(argv)
    if args.action != "collect" and any((args.start_day, args.end_day, args.ca_bundle)):
        parser.error("Les dates et --ca-bundle sont exclusivement reserves a Collect.")
    ...
    try:
        ...
        if args.action == "collect":
            result = runner.collect(config, root=ROOT, start_day=args.start_day, end_day=args.end_day, ca_bundle=args.ca_bundle)
        ...
        logging.error("[Physical P50] %s", exc)
        return 1
