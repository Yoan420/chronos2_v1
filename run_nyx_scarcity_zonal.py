"""Separate zonal backtests/reports, with no operational writes or promotion."""
...
def main(argv=None):
    ...
    parser.add_argument("--config", type=Path, default=ROOT/"config/nyx_scarcity_zonal.yaml")
    parser.add_argument("--run-directory", type=Path)
    parser.add_argument("--variant")
    args = parser.parse_args(argv)
    logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
    try:
        from nyx_scarcity_zonal import runner
        ...
        logging.exception("Zonal experiment failed. Existing operational forecasts are unchanged.")
        return 2
