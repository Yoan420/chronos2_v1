"""Dated, isolated live shadow training: DE/NL first, independent BE/FR next."""
...
from chronos2_hourly.nyx_live_sources import OUTPUT, safe, atomic_json, ensure_live_sources
...
CODE = ("run_nyx_test2_live.py", "chronos2_hourly/nyx_live_sources.py",
...
def build_plan(day):
    ...
    verify_reference()
    return {"schema_version": 1, "engine": "nyx_test2_live_v1", "delivery_day": day,
        ...
        "threads": 1, "workers": 1, "device": "cpu", "priority": "BelowNormal",
...
class Run:
    def __init__(self, day, expected):
        ...
        self.state = {"engine": self.plan["engine"], "delivery_day": day,
            ...
            "command": [sys.executable, *sys.argv], "threads": 1, "workers": 1,
    ...
    def status(self, phase, **extra):
        ...
        print(f"[NYX live] {phase}: {json.dumps(extra, default=str, ensure_ascii=False)}", flush=True)
    ...
    def execute(self):
        from chronos2_hourly.nyx_live_baseline import run_zone
        from chronos2_hourly.nyx_live_hybrid import build_pair_interaction
        if os.name == "nt":
            psutil.Process().nice(psutil.BELOW_NORMAL_PRIORITY_CLASS)
        self.status("capture_sources")
        self.resource_guard()
        ...
        for pair in PAIR_ORDER:
            ...
            for zone in pair:
                ...
                result = run_zone(zone, self.day, donor, sources, workroot=OUTPUT,
                    threads=1, workers=1, device="cpu", action="run", interaction_builder=build_pair_interaction)
            ...
            temporary.write_text(f'<!doctype html><meta charset="utf-8"><title>NYX / Test2 {self.day}</title><h1>NYX / Test2 — {self.day}</h1><p>Candidat isolé, production inchangée. BE/FR : extension distincte.</p><ul>{links}</ul>', encoding="utf-8")
