"""NYX + four raw CWE solar forecasts; isolated replay of the unchanged engines."""
...
OUTPUT = ROOT / "runs/experiments/solar_cwe_v1"
...
def safe(path):
    ...
    if OUTPUT.resolve() != OUTPUT or path != path.resolve() or not path.is_relative_to(OUTPUT) or path == OUTPUT:
        raise ValueError("Solar writes must stay in a real child of runs/experiments/solar_cwe_v1.")
    return path


def load_settings(path):
    cfg = yaml.safe_load(Path(path).read_text(encoding="utf-8-sig"))
    fields = {"schema_version", "delivery_day", "zones", "output_root", "source_root", "include_attribution",
    ...
    if not isinstance(cfg, dict) or set(cfg) != fields or type(cfg["schema_version"]) is not int or cfg["schema_version"] != 1:
        raise ValueError("Invalid SolarCWE configuration.")
...
def validate_settings(cfg):
    ...
    if not isinstance(zones, list) or not zones or len(zones) != len(set(zones)) or set(zones)-set(ZONES):
        raise ValueError("Choose unique target countries FR, DE, BE, NL.")
...
def identity(cfg, zone, sources):
    ...
    return {"zone": zone, "delivery_day": cfg["delivery_day"], "settings": cfg,
        "input_protocol": solar_cwe_input_protocol(), "runner_sha256": sha256(Path(__file__)),
...
def verify_snapshot(work, *, zone=None, delivery_day=None):
    ...
    if (saved_zone not in ZONES or not isinstance(saved_day, str)
            ...
            or signature.get("input_protocol") != solar_cwe_input_protocol()):
...
def prepare(cfg, zone, sources):
    ...
    from chronos2_hourly.solar_cwe_forecast import SOLAR_SERIES, solar_cwe_input_protocol
    if set(sources) != set(SOLAR_SERIES):
        raise ValueError("Exactly the four CWE solar forecasts are required.")
    ...
    with exclusive_process_lock(work/"prepare.lock"):
        ...
        if set(paths) != set(mapping) or set(mapping).intersection(SOLAR_SERIES):
        ...
        for alias, record in sources.items():
            ...
            for source_file in (source, Path(str(source)+".audit.json")):
                copied = _copy_checked(source_file, safe(work/"snapshot"/source_file.name))
                files.append(copied)
        ...
        for alias, series in SOLAR_SERIES.items():
            spec["covariates"][alias] = {"enabled": True, "source": "pit_parquet", "series": series,
                "description": f"{alias[:2].upper()} generation solaire prevue horaire (GW)",
        ...
        config["report"]["title"] = f"NYX nucleaire FR + solaire CWE — {zone}"
        ...
        experiment.update(input_protocol=solar_cwe_input_protocol(), mode="incremental",
            incremental_cache_dir=str(OUTPUT/"_daily_cache"/zone.lower()), candidate_model="solar_cwe", production_modified=False)
        ...
        audit = {"solar": sources, "baseline": str(baseline), "same_history_anchor": anchor,
            "same_model_hyperparameters": True, "added_inputs_only": list(SOLAR_SERIES),
            "no_custom_ramp_or_spike_expert": True, "solar_already_implicit_in_residual_load": True,
...
def run_zone(cfg, zone, work, args):
    ...
    from chronos2_hourly.solar_cwe_forecast import run_solar_cwe_forecast
    from chronos2_hourly.solar_cwe_reporting import render_solar_cwe_reports
    ...
    with exclusive_process_lock(work/"run.lock"), run_progress(work, zone, pd.Timestamp(cfg["delivery_day"]), args.action) as progress:
        ...
        if args.action == "report" or reuse:
            ...
            progress("forecast", solar_in_chronos=True, solar_in_residual=True, solar_in_kalman=True)
            print(f"[SolarCWE/{zone}] Nouveau replay Chronos + residuel + Kalman, avec seulement les quatre courbes solaires.", flush=True)
            result = run_forecast_with_storage_retry(run_solar_cwe_forecast, config=config, data=data, zone=zone,
        ...
        reports = render_solar_cwe_reports(result, data=replace(data, target=actual), zone=zone,
...
def main(argv=None):
    ...
    parser.add_argument("--config", type=Path, default=ROOT/"config/solar_cwe.yaml")
    ...
    if args.action == "report":
        ...
        sources = ensure_solar_sources(output_root=SOURCE, start_day=audit["start_day"], end_day=audit["end_day"],
                                       workers=args.workers, sync=True)
        if args.action == "sync":
    ...
    if args.action != "prepare":
        index = safe(pointer.parent/("solar_cwe_"+"_".join(cfg["zones"])+"_index.html"))
        links = []
        ...
        index.write_text('<!doctype html><meta charset="utf-8"><title>NYX Solar CWE</title>'
            '<style>body{font:18px system-ui;max-width:900px;margin:4rem auto;background:#111827;color:#e5e7eb}'
            'a{color:#7dd3fc}li{margin:1rem}</style><h1>NYX nucléaire FR + solaire CWE</h1>'
            '<p>Quatre courbes solaires horaires dans Chronos-2, le correcteur et Kalman. Variante séparée ; production inchangée.</p><ul>'
...
if __name__ == "__main__":
    try:
        ...
        logging.exception("[SolarCWE] %s — production unchanged", exc)
        raise SystemExit(1)
