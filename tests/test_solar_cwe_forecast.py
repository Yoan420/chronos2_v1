...
from chronos2_hourly import solar_cwe_forecast as solar
...
def inputs(tmp_path, *, day="2026-09-18", zone="FR", raw_days=730):
    ...
    for number, alias in enumerate(solar.INPUT_ALIASES):
        ...
        values = (np.maximum(0, np.sin(index.tz_convert(tz).hour.to_numpy()/24*2*np.pi-np.pi/2))*(number+1)
                  if alias in solar.SOLAR_ALIASES else np.full(len(index), 30.+number))
    ...
    for alias, series in solar.SOLAR_SERIES.items():
    ...
    config = {"data": {"project_root": str(tmp_path), "forecast_origin_local_time": "08:00"},
              ...
              "nuclear_experiment": {"input_protocol": solar.solar_cwe_input_protocol(),
    ...
    return config, data, delivery, tmp_path/"runs/experiments/solar_cwe_v1/w", required
...
class FakeResidual:
    ...
    def fit(self, X, actual, prediction, experts):
        self.feature_columns_ = (*solar.SOLAR_KNOWN_COLUMNS, base.NUCLEAR_KNOWN_COLUMN)
...
def fake_base(**kwargs):
    ...
    daily = pd.DataFrame({"generation_source": ["identity_chronos_cold_start", "daily_prequential_refit"],
                         "residual_feature_columns": [[], [*solar.SOLAR_KNOWN_COLUMNS, base.NUCLEAR_KNOWN_COLUMN]]})
...
def test_four_hourly_inputs_consumed_three_stages_without_mutating_baseline(tmp_path, monkeypatch, zone):
    ...
    result = solar.run_solar_cwe_forecast(config=config, data=data, zone=zone, delivery_day=day, workdir=work,
    ...
    for alias in solar.SOLAR_ALIASES:
    ...
    assert result.audit["additional_raw_input_count"] == 4 and result.audit["new_ramps_or_aggregates"] is False
    assert result.audit["candidate_engine"] == "solar_cwe_v1"
    assert result.audit["solar_chronos_known_future_columns"] == list(solar.SOLAR_KNOWN_COLUMNS)
...
def test_invalid_contract_fails_before_any_model_or_write(tmp_path, monkeypatch, fault):
    config, data, day, work, _ = inputs(tmp_path)
    alias, known = solar.SOLAR_ALIASES[0], solar.SOLAR_KNOWN_COLUMNS[0]
    ...
    def forbidden(**kwargs):
        pytest.fail("Invalid SolarCWE contract reached the model")
    monkeypatch.setattr(base, "run_nuclear_forecast", forbidden)
    with pytest.raises(ValueError):
        solar.run_solar_cwe_forecast(config=config, data=data, zone="FR", delivery_day=day, workdir=work)
    assert not work.exists()


def test_schema_protocol_and_kalman_dimensionality(tmp_path, monkeypatch):
    previous = solar.solar_cwe_input_protocol()
    monkeypatch.setitem(solar.SOLAR_SERIES, "fr_solar_generation_fcst", "changed-series")
    assert solar.solar_cwe_input_protocol() != previous
    original = base.nuclear_kalman_covariate_config()
    contract = solar.solar_cwe_kalman_covariate_config(original)
    assert original.derived == contract.derived and len(contract.input_columns) == 10
    assert contract.minimum_history_coverage == 1 and contract.require_future_complete is True
    assert not set(solar.SOLAR_ALIASES).intersection(original.input_columns)
    ...
    with pytest.raises(ValueError, match="maximum"):
        solar.solar_cwe_kalman_covariate_config(huge)
...
def test_default_kalman_normalizes_timestamp_and_does_not_add_cache_namespace(tmp_path, monkeypatch):
    ...
    result = solar.run_solar_cwe_forecast(config=config, data=data, zone="FR", delivery_day=day, workdir=work,
    ...
    assert result.audit["solar_kalman_delegate_signature"] is None
...
def test_real_native_kalman_accepts_solar_columns_before_replay(tmp_path, monkeypatch):
    ...
    def validated(*args, **kwargs):
        assert "timestamp" in kwargs["covariates"] and set(solar.SOLAR_ALIASES).issubset(kwargs["covariates"])
    ...
    hook = solar._SolarKalmanBuilder(data.model_context_covariates, native.build_operational_kalman_view, None)
...
class FakeForecasting:
    ...
    def _check_neural_frames(data, origin, context_length, horizon, item, live=False):
        ...
        assert set(solar.SOLAR_ALIASES).issubset(context)
        assert set(solar.SOLAR_KNOWN_COLUMNS).issubset(context) and set(solar.SOLAR_KNOWN_COLUMNS).issubset(future)
    ...
    def run_backtest_variant(cls, **kwargs):
        ...
        for origin in kwargs["origins"]:
            ...
            middle = sum(data.model_context_covariates.loc[index, known] for known in solar.SOLAR_KNOWN_COLUMNS)
...
def test_real_730_day_base_engine_and_effective_neural_inputs(tmp_path_factory):
    ...
    expected = sum(data.model_context_covariates[alias].iloc[-24:].to_numpy() for alias in solar.SOLAR_ALIASES)
    np.testing.assert_allclose(result.source_forecast.chronos2__q50, expected, rtol=1e-6, atol=1e-6)
    expected += .01*data.model_context_covariates[solar.SOLAR_ALIASES[0]].iloc[-24:].to_numpy()
