...
for name,size,space in [('Title',19,10),('Heading 1',13,11),('Heading 2',11.8,9),('Heading 3',11.3,8)]:
...
def formula_xml(latex,display=False):
    result=OxmlElement('m:oMath')
    # Office's import drops MathML spaces and treats wide accents as upper limits.
    # Preserve mathematical spacing and build proper native Word accents instead.
    latex=latex.replace(r'\operatorname{',r'\mathrm{')
    for part in re.split(r'(\\qquad|\\quad)',latex):
        if part in [r'\quad',r'\qquad']:
            r=OxmlElement('m:r');t=OxmlElement('m:t');t.set(qn('xml:space'),'preserve');t.text='\u2003'*(2 if part==r'\qquad' else 1);r.append(t);result.append(r)
        elif part.strip():
            converted=XSLT(etree.fromstring(convert(part.strip(),display='block' if display else 'inline').encode())).getroot()
            if converted.tag==qn('m:oMathPara'):converted=converted.find(qn('m:oMath'))
            for upper in list(converted.iter(qn('m:limUpp'))):
                limit=upper.find(qn('m:lim'))
                label=''.join(limit.itertext()) if limit is not None else ''
                accents={'^':'\u0302','~':'\u0303','¯':'\u0305','ˉ':'\u0305'}
                if label in accents:
                    acc=OxmlElement('m:acc');pr=OxmlElement('m:accPr');ch=OxmlElement('m:chr');ch.set(qn('m:val'),accents[label]);pr.append(ch);acc.append(pr);acc.append(upper.find(qn('m:e')));upper.getparent().replace(upper,acc)
            for child in list(converted):result.append(child)
    return result

def math(p,latex,display=False):
    tag=re.search(r'\\tag\{([^}]+)\}',latex)
    latex=re.sub(r'\\tag\{[^}]+\}','',latex).strip()
    if display and r'\begin{aligned}' in latex:
        body=latex.split(r'\begin{aligned}',1)[1].split(r'\end{aligned}',1)[0]
        formula=OxmlElement('m:oMath');arr=OxmlElement('m:eqArr')
        for line in re.split(r'\\\\',body):
            if not line.strip():continue
            row=OxmlElement('m:e')
            for child in list(formula_xml(line.replace('&',''),True)):row.append(child)
            arr.append(row)
        formula.append(arr)
    else:formula=formula_xml(latex,display)
    if tag:
...
tabledef('data_description','Table 1. Descriptive statistics of the evaluated hourly price series',['Zone','Mean','SD','Minimum','Median','Maximum','Negative hours'],[[r.zone,*[f'{v:.2f}' for v in [r['mean'],r.sd,r['min'],r['median'],r['max']]],str(r.negative_hours)] for _,r in data.iterrows()],[.5,.8,.8,.9,.85,.9,1.15],'Prices are in EUR/MWh; sample standard deviation. Each country has 8,400 observations over 350 delivery days. Negative hours have an observed price below zero.')
...
['Governed filter','Required improvement; raw cap','max(0.05 EUR/MWh, 0.5% of upstream MAE); ±20 EUR/MWh']],[1.05,2.05,3.6],'There is no LoRA or foundation-model fine-tuning. Section 2.3 specifies filter dynamics. Settings are reported as executed, without treating unused configuration entries as active hyperparameters.')
s=pd.read_csv(SRC/'verified_stage_metrics.csv');s=s[(s.period=='full350')&(s.support=='four_model_common')]
zones=['BE','DE','FR','NL','pooled']
def val(z,model,col):return s[(s.zone==z)&(s.model==model)].iloc[0][col]
tabledef('annual_stages','Table 4. Evaluation period MAE across the retained NYX stages',['Zone','Chronos-2','After CatBoost','NYX','MAE reduction (%)'],[[z.upper() if z=='pooled' else z,fmt(val(z,'chronos','mae')),fmt(val(z,'residual','mae')),fmt(val(z,'nyx','mae')),f"{100*(1-val(z,'nyx','mae')/val(z,'chronos','mae')):.2f}"] for z in zones],[.7,1.35,1.45,1.25,1.55],'MAE in EUR/MWh. All three stages use the same 8,399 hours per country as Table 5. Reduction is relative to the Chronos-2 stage. Pooled n = 33,596.')
tabledef('annual_benchmark','Table 5. Evaluation period NYX and Storm reporting-composite performance',['Zone','NYX MAE','Storm MAE','NYX RMSE','Storm RMSE','NYX bias','Storm bias'],[[z.capitalize() if z=='pooled' else z,*[fmt(val(z,mod,metric)) for metric in ['mae','rmse','bias'] for mod in ['nyx','storm']]] for z in zones],[.64,1.01,1.01,1.01,1.01,1.01,1.01],'All errors in EUR/MWh on 33,596 paired observations. Bias = forecast − observation. Storm includes recomputed fallback hours; Table 9 checks sensitivity to their exclusion.')
...
tabledef('residual_test','Table 8. Validation-selected residual configurations on the reserved 75 days',['Model and selected setting','Validation MAE','Test MAE','Test RMSE','Test bias','ΔMAE [95% interval]'],rows,[1.7,.85,.82,.85,.78,1.6],'n = 7,200 country–hours in the reserved segment. Errors in EUR/MWh; positive ΔMAE means worse than NYX. Validation uses frozen labels; test uses refreshed labels. Paired moving-block intervals use 2,000 replicates and seven-day blocks. Storm is not part of residual-model selection.')
p=pd.read_csv(SRC/'storm_provenance_sensitivity.csv');p=p[p.zone=='pooled'];rows=[]
for period,label in [('full350','350 days'),('test75','Reserved 75 d')]:
    for subset,sl in [('all_common','All common hours'),('cache_only','Exclude Storm recomputation')]:
...
tabledef('provenance_sensitivity','Table 9. Pooled MAE sensitivity to source provenance',['Period','Included support','Pairs','NYX MAE','Storm MAE'],rows,[1.1,2.3,1,1.15,1.05],'Errors in EUR/MWh. Samples are matched within each row and weighted by country–hour. No EPEX observed-label fallback lies within the retained dates. Excluding fallback values does not independently certify the remaining source vintages.')
...
for block in re.split(r'\n\s*\n',source.strip()):
    ...
    if block.startswith('# '):doc.add_paragraph(block[2:],style='Title')
        ...
        if doc.paragraphs and doc.paragraphs[-1].style.name=='Normal':doc.paragraphs[-1].paragraph_format.keep_with_next=True
        p=doc.add_paragraph();p.alignment=WD_ALIGN_PARAGRAPH.CENTER;p.paragraph_format.space_after=Pt(9);p.paragraph_format.space_before=Pt(6);p.paragraph_format.keep_together=True;math(p,block[2:-2],display=True)
    else:
...
for name in ['verified_stage_metrics.csv','stage_paired_block_bootstrap.csv','weekly_naive_matched_metrics.csv','weekly_naive_paired_bootstrap.csv','storm_provenance_sensitivity.csv','source_fallback_counts.csv','verified_interval_metrics.csv']:
    shutil.copyfile(SRC/name,OUT/'tables'/name)
