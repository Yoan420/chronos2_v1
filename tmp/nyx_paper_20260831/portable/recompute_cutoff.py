...
ROOT=args.workspace.resolve(); SCRIPT_DIR=Path(__file__).resolve().parent
OUT=SCRIPT_DIR/'recomputed'
...
# Aggregate-only supplement: the filtered raw forecast matrix remains in memory.
# Saved test predictions are read locally but are not redistributed as output.
...
tables=load(SCRIPT_DIR/'table_layout_seed.json')
