...
for p in manifest['provenance']:
    z=p['zone']; w=Path(p['workspace']); b=w/'report_only/frozen_result'
    ...
    src=Path(p['source_audit_path']); source_audit=load(src)
...
for z,g in f.groupby('zone'):
    ...
    audit['material_label_revisions'].append({'zone':z,'count':int(mat.sum()),'max_abs':float(diff.max()),'mean_abs':float(diff.mean()),
      'records':g.loc[mat,['timestamp','day','observed','frozen_actual']].astype({'timestamp':str}).to_dict('records'),
