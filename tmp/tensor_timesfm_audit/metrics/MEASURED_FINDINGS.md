...
Reproduction depuis la racine du projet, dans l'environnement Python du projet : `python tmp/tensor_timesfm_audit/metrics/extract_metrics.py`, puis `python tmp/tensor_timesfm_audit/metrics/experiments.py`. Ces scripts lisent les bundles et écrivent uniquement dans ce dossier. Ils ne contactent aucune API de marché et n'entraînent aucun modèle de production.
